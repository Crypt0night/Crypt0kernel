...
void send_cal(int port_id, struct acdb_cal_block *aud_cal)
{
	...
	if (result < 0) {
		pr_aud_err("%s: Set params failed port = %d payload = 0x%x\n",
...
void send_adm_cal(int port_id, int path)
{
	...
	if ((acdb_path >= NUM_AUDPROC_BUFFERS) ||
		(acdb_path < 0)) {
		pr_aud_err("%s: Path is not RX or TX, path = %d\n",
			__func__, path);
		goto done;
	}
				...
				uint32_t *bufsz, uint32_t bufcnt)
{
	...
	if (ret < 0) {
		pr_aud_err("%s: mmap_regions op[0x%x]rc[%d]\n", __func__,
	...
	if (!ret) {
		pr_aud_err("%s: timeout. waited for memory_map\n", __func__);
		ret = -EINVAL;
		goto fail_cmd;
	}
fail_cmd:
						...
						uint32_t bufcnt)
{
	...
	ret = wait_event_timeout(this_adm.wait,
			atomic_read(&this_adm.copp_stat[0]), 5 * HZ);
	if (!ret) {
		pr_aud_err("%s: timeout. waited for memory_unmap\n", __func__);
		ret = -EINVAL;
		goto fail_cmd;
	}
fail_cmd:
...
int adm_close(int port_id)
{
	...
	if (!(atomic_read(&this_adm.copp_cnt[index]))) {
		...
		ret = wait_event_timeout(this_adm.wait,
				...
				msecs_to_jiffies(TIMEOUT_MS));
		if (!ret) {
			pr_aud_info("%s: ADM cmd Route failed for port %d\n",
							__func__, port_id);
			ret = -EINVAL;
			goto fail_cmd;
		}
