...
int afe_open(u16 port_id, union afe_port_config *afe_config, int rate)
{
	...
	if (!afe_config) {
		pr_aud_err("%s: Error, no configuration data\n", __func__);
		ret = -EINVAL;
		return ret;
	}
	...
	if (ret < 0) {
		pr_aud_err("%s: AFE enable for port %d failed\n", __func__,
				port_id);
		ret = -EINVAL;
		goto fail_cmd;
	}
...
int afe_loopback(u16 enable, u16 rx_port, u16 tx_port)
{
	...
	if (ret < 0) {
		pr_aud_err("AFE loopback failed\n");
		ret = -EINVAL;
		goto done;
	}
	ret = wait_event_timeout(this_afe.wait,
	...
	const char __user *ubuf, size_t cnt, loff_t *ppos)
{
	...
	if (!strcmp(lb_str, "afe_loopback")) {
		...
		if (!rc) {
			pr_aud_info("%s %lu %lu %lu\n", lb_str, param[0], param[1],
				param[2]);

			if ((param[0] != AFE_LOOPBACK_ON) && (param[0] !=
				AFE_LOOPBACK_OFF)) {
				pr_aud_err("%s: Error, parameter 0 incorrect\n",
					__func__);
				rc = -EINVAL;
				goto afe_error;
			}
...
int afe_sidetone(u16 tx_port_id, u16 rx_port_id, u16 enable, uint16_t gain)
{
	...
	pr_aud_info("%s: tx_port_id:%d rx_port_id:%d enable:%d gain:%d\n", __func__,
	...
	if (ret < 0) {
		pr_aud_err("%s: AFE sidetone failed for tx_port:%d rx_port:%d\n",
...
int afe_close(int port_id)
{
	...
	if (this_afe.apr == NULL) {
		pr_aud_err("AFE is already closed\n");
		ret = -EINVAL;
		goto fail_cmd;
	}
